...
@Slf4j
public class ServiceBuilder extends TemplateBuilder {

    @Override
    protected Template getTemplate() {
        Configuration config = super.getConfiguration();
        try {
            return config.getTemplate(TemplateEnum.SERVICE.templateName);
        } catch (Exception e) {
            throw new RuntimeException("Template not found: " + TemplateEnum.SERVICE.templateName, e);
        }
    }
    ...
    protected Map<String, Object> getDataModel(TableInfo tableInfo) {
        ...
        dataModel.put("className", tableInfo.getTableName() + "Service");

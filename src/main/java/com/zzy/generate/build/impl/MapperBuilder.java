...
@Slf4j
public class MapperBuilder extends TemplateBuilder {

    @Override
    protected Template getTemplate() {
        Configuration config = super.getConfiguration();
        try {
            return config.getTemplate(TemplateEnum.MAPPER.templateName);
        } catch (Exception e) {
            throw new RuntimeException("Template not found: " + TemplateEnum.MAPPER.templateName, e);
        }
    }
